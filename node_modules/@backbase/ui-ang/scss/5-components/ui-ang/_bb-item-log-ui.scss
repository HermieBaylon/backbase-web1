...
$item-log-bullet-margin-top: $spacer-sm !default;
...
.bb-item-log-record__bullet-icon {
  ...
  margin-top: calc(#{$item-log-bullet-margin-top} - #{$item-log-bullet-border-width});
...
.item-log__record--has-bullet {
  &::before {
    top: calc((#{$item-log-bullet-size} - #{$item-log-bullet-border-width}) * 2);
    left: calc((#{$item-log-bullet-size} - #{$item-log-bullet-border-width}) / 2);
    height: calc(100% - #{$item-log-bullet-size});
    border-left: $item-log-bullet-border-width $border-style $item-log-bullet-color;
  }
}
