// Global annotations
////
/// @group forms
/// @access private
////

/// Set the box-shadow for focused search box
$search-box-focus-box-shadow: $input-focus-box-shadow !default;

/// Set the border for focused search box
$search-box-focus-border: $input-focus-border !default;

$search-box-small-search-icon-vertical-adjustment: 2px !default;

/// @todo Bootstrap has a nested !important on the hover which overrules this styling
$search-box-button-hover-color: $color-neutral-darker !default;

.bb-search-box-visible {
  // @deprecated - use .bb-search-box--has-search
  &.bb-search-box-search input {
    padding-left: $search-box-padding-horizontal;
    border-top-right-radius: $search-box-border-radius;
    border-bottom-right-radius: $search-box-border-radius;
  }

  // @deprecated - use .bb-search-box--has-clear
  &.bb-search-box-clear input {
    padding-right: $search-box-padding-horizontal;
  }

  input {
    order: 1;

    &[type="search"] {
      &::-ms-clear,
      &::-ms-reveal {
        display: none;
        width: 0;
        height: 0;
      }

      &::-webkit-search-decoration,
      &::-webkit-search-cancel-button,
      &::-webkit-search-results-button,
      &::-webkit-search-results-decoration {
        -webkit-appearance: none;
      }
    }

    &:focus {
      box-shadow: $search-box-focus-box-shadow;
    }
  }

  .input-group-prepend {
    order: 0;
  }

  .input-group-append {
    order: 2;
  }

  .input-group-prepend button,
  .input-group-append button {
    color: $search-box-button-color;

    bb-icon-ui {
      vertical-align: bottom;
    }

    &:focus {
      box-shadow: $search-box-focus-box-shadow;
    }
  }

  &:not(.bb-search-box--has-clear) .bb-search-box__input {
    @include border-right-radius($search-box-border-radius);
    &.open {
      @include border-right-radius($search-box-border-radius);
    }
  }

  &:not(.bb-search-box--has-search) .bb-search-box__input {
    @include border-left-radius($search-box-border-radius);
  }
}

.bb-search-box--has-search {
  .bb-search-box__input {
    padding-left: $search-box-padding-horizontal;
  }

  .bb-search-box__search-button button {
    @include border-left-radius($search-box-border-radius);
    padding-right: $search-box-button-padding-horizontal;
    padding-left: $search-box-button-padding-horizontal;

    &.btn-link:not(:disabled):not(.disabled):hover {
      color: $search-box-button-hover-color;
      background: $search-box-button-background-color;
    }

    &.btn-link:not(:disabled):not(.disabled):focus {
      color: $search-box-button-hover-color;
      background: $search-box-button-background-color;
    }

    &.btn-sm {
      bb-icon-ui[name="search"] {
        margin-top: -1 * $search-box-small-search-icon-vertical-adjustment;
      }
    }
    //@deprecated Deprecated as part of the 2022.02 Release:
    //This feature is by default set to true which mean as standard the theme uses the new improved more visible focus for accessibility but can be set to false to revert to the previous less accessible version.
    //This flag will be removed in future major release 2022.09, post which this flag will not be available any more to disable this feature.
    @if $experimental-enable-focus {
      &.btn:focus::before {
        @include border-right-radius(0);
        right: $search-focus-outline-distance/4;
      }
    }
  }

  &.rounded {
    .bb-search-box__search-button button {
      @include border-left-radius($border-radius-circle);
    }
  }
}

.bb-search-box--has-clear {
  .bb-search-box__input {
    padding-right: $search-box-padding-horizontal;
  }

  .bb-search-box__clear-button button {
    @include border-right-radius($search-box-border-radius);

    padding-right: $search-box-button-padding-horizontal;
    padding-left: $search-box-button-padding-horizontal;

    &.btn-link:not(:disabled):not(.disabled):hover {
      color: $search-box-button-hover-color;
      background: $search-box-button-background-color;
    }

    &.btn-link:not(:disabled):not(.disabled):focus {
      color: $search-box-button-hover-color;
      background: $search-box-button-background-color;
    }
    //@deprecated Deprecated as part of the 2022.02 Release:
    //This feature is by default set to true which mean as standard the theme uses the new improved more visible focus for accessibility but can be set to false to revert to the previous less accessible version.
    //This flag will be removed in future major release 2022.09, post which this flag will not be available any more to disable this feature.
    @if $experimental-enable-focus {
      &.btn:focus::before {
        @include border-left-radius(0);
        left: $search-focus-outline-distance/4;
      }
    }
  }

  &.rounded {
    .bb-search-box__clear-button button {
      @include border-right-radius($border-radius-circle);
    }
  }
}

.bb-search-box__search-button {
  margin-bottom: 0;
}

.bb-search-box {
  &:not(.bb-search-box--has-clear) {
    .bb-search-box__input {
      &.open {
        border-right-width: $input-border-width;
        @include border-right-radius($input-border-radius);
      }
    }
  }
}
