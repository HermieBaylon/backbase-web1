(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    var multipleAccountsPredicate = function (listOptions) { return function () { return Array.isArray(listOptions.account.arrangementId); }; };
        ...
        function TransactionsService(transactionsDataHttpService, accountService, deprecationsService, locale) {
            ...
            this.transactionsList = rxjs.merge(this.listOptions.pipe(operators.filter(function (listOptions) { return typeof listOptions.account.arrangementId !== 'undefined'; }), operators.distinctUntilChanged(deepEqual)), this.listRefresh).pipe(cacheRequest(function (listOptions) {
                ...
                return rxjs.combineLatest([
                    ...
                    }), operators.map(function (transactionsList) { return ({ transactionsList: transactionsList, onLoad: listOptions.onLoad }); })),
        ...
        function PendingTransactionsService(transactionsDataHttpService, accountService, deprecationsService) {
            ...
            this.transactionsList = rxjs.merge(this.listOptions.pipe(operators.filter(function (listOptions) { return typeof listOptions.account.arrangementId !== 'undefined'; }), operators.distinctUntilChanged(deepEqual)), this.listRefresh).pipe(cacheRequest(function (listOptions) {
                ...
                return rxjs.combineLatest([
                    _this.loadTransactions(listOptions).pipe(operators.tap(function () {
                        _this.loadingState.next(exports.LoadingState.Loaded);
                    }), operators.map(function (transactionsList) { return ({ transactionsList: transactionsList, onLoad: exports.OnLoadAction.ReplacePage }); })),
                    rxjs.iif(multipleAccountsPredicate(listOptions), _this.accountService.getAllArrangements().pipe(operators.map(function (accounts) {
                        var accountMap = new Map();
                        accounts.forEach(function (account) {
                            accountMap.set(account.id, account);
    ...
    var TransactionDetailsService = /** @class */ (function () {
        ...
        TransactionDetailsService.prototype.fromCheckImages = function (hasCheckImages) {
            ...
            return this.setCondition(this.transactionSelection, this.hasCheckImages).pipe(operators.tap(function () {
                _this.transactionsDetailsStatus.next({ checkImages: exports.TransactionsDetailsLoadingState.loading });
                ...
                return _this.transactionsDataHttpService.getTransactionCheckImages({ transactionId: transactionId }, 'response').pipe(operators.tap(function () {
                    _this.transactionsDetailsStatus.next({ checkImages: exports.TransactionsDetailsLoadingState.done });
                }), operators.catchError(function (error) {
                    switch (error.status) {
                        case 500: {
                            _this.transactionsDetailsStatus.next({ checkImages: exports.TransactionsDetailsLoadingState.serverError });
                            break;
                        }
                        case 400: {
                            _this.transactionsDetailsStatus.next({ checkImages: exports.TransactionsDetailsLoadingState.badRequestError });
                            break;
                        }
                        case 404: {
                            _this.transactionsDetailsStatus.next({ checkImages: exports.TransactionsDetailsLoadingState.notFoundError });
                            break;
                        }
                        default: {
                            _this.transactionsDetailsStatus.next({ checkImages: exports.TransactionsDetailsLoadingState.error });
                        }
                    }
        ...
        TransactionDetailsService.prototype.fromGeolocation = function (mapsApiKey) {
            ...
            return rxjs.combineLatest([this.transactionSelection, this.mapsApiKey]).pipe(operators.map(function (_b) {
                ...
                _this.transactionsDetailsStatus.next({ geolocation: exports.TransactionsDetailsLoadingState.loading });
                if (!apiKey || !transaction.location || !transaction.location.latitude || !transaction.location.longitude) {
                    _this.transactionsDetailsStatus.next({ geolocation: exports.TransactionsDetailsLoadingState.notLoaded });
                    return Promise.resolve(false);
                }
                return _this.mapApiLoader.load().then(function () {
                    _this.transactionsDetailsStatus.next({ geolocation: exports.TransactionsDetailsLoadingState.done });
                    return true;
                }, function () {
                    _this.transactionsDetailsStatus.next({ geolocation: exports.TransactionsDetailsLoadingState.error });
                    return false;
                });
